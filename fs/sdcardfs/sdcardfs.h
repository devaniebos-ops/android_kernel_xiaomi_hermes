...
#define fixup_tmp_permissions(x)	\
	do {						\
		(x)->i_uid = make_kuid(current_user_ns(), SDCARDFS_I(x)->data->d_uid);	\
		(x)->i_gid = make_kgid(current_user_ns(), AID_SDCARD_RW);	\
...
static inline void sdcardfs_copy_and_fix_attrs(struct inode *dest, const struct inode *src)
{
	...
	dest->i_uid = make_kuid(current_user_ns(), SDCARDFS_I(dest)->data->d_uid);
    dest->i_gid = make_kgid(current_user_ns(), AID_SDCARD_RW);
