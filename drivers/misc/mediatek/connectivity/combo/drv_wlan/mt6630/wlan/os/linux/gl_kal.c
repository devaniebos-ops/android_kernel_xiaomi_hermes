...
WLAN_STATUS kalFirmwareOpen(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	orgfsuid = from_kuid(current_user_ns(), cred->fsuid);
    orgfsgid = from_kgid(current_user_ns(), cred->fsgid);
...
error_open:
	/* restore */
	set_fs(orgfs);
	cred->fsuid = make_kuid(current_user_ns(), orgfsuid);
    cred->fsgid = make_kgid(current_user_ns(), orgfsgid);
	put_cred(cred);
		...
		if (prGlueInfo->fgIsRegistered == TRUE) {
			...
			if (bss == NULL) {
				...
				if (!prChannel) {
 					...
 					DBGLOG(INIT, WARN, "ChnlNum %d, bssIndex %d, bss is same %d\n", ucChannelNum,
                        			prAdapter->prAisBssInfo->ucBssIndex,
                        			GET_BSS_INFO_BY_INDEX(prAdapter, prAdapter->prAisBssInfo->ucBssIndex)==prAdapter->prAisBssInfo);
	                     }
